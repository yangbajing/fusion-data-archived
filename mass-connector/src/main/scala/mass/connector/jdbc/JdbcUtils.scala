...
object JdbcUtils extends StrictLogging {
  ...
  @inline def createHikariDataSource(config: Configuration): HikariDataSource =
    createHikariDataSource(new HikariConfig(config.getProperties(null)))

  @inline def createHikariDataSource(config: Config): HikariDataSource = createHikariDataSource(Configuration(config))

  @inline def createHikariDataSource(props: Properties): HikariDataSource = createHikariDataSource(new HikariConfig(props))
