...
class JdbcTemplate private (
  ...
  override def source(creator: ConnectionPreparedStatementCreator, batchSize: Int): Source[ResultSet, NotUsed] =
    JdbcSource(creator, batchSize)(dataSource)
  ...
  override def sink[T](
    ...
    JdbcSink(creator, action, batchSize)(dataSource)
...
object JdbcTemplate {
  ...
  def apply(
      ...
      _useTransaction: Boolean = true,
      ignoreWarnings: Boolean = true,
      _allowPrintLog: Boolean = false
  ): JdbcOperations = new JdbcTemplate(dataSource, _useTransaction, ignoreWarnings, _allowPrintLog)
