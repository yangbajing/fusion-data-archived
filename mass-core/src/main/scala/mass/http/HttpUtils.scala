...
object HttpUtils extends StrictLogging {
  ...
  def mapHttpResponseError[R](
    ...
    if (response.entity.contentType.mediaType == MediaTypes.`application/json`) {
      ...
      Unmarshal(response.entity)
        .to[String]
        .map(errMsg =>
          Left(new HSException(response.status.intValue(), errMsg)))(
          if (ec eq null) mat.executionContext else ec)
    }
  }
  ...
  /**
    * 根据 Content-Type 字符串解析转换成 [[akka.http.scaladsl.model.ContentType]]
    *
  ...
  private def tupleKeyToContentType(charset: String,
  ...
  def cachedHostConnectionPool(url: String)(
      implicit mat: ActorMaterializer): HttpSourceQueue = {
    val uri = Uri(url)
    uri.scheme match {
      case "http" =>
        cachedHostConnectionPool(uri.authority.host.address(),
                                 uri.authority.port)
      case "https" =>
        cachedHostConnectionPoolHttps(uri.authority.host.address(),
                                      uri.authority.port)
      case _ =>
  ...
  /**
    ...
    * @return
    */
  def cachedHostConnectionPool(host: String, port: Int = 80)(
      implicit mat: ActorMaterializer): HttpSourceQueue = {
    implicit val system = mat.system
    val poolClientFlow =
      Http().cachedHostConnectionPool[Promise[HttpResponse]](host, port)
    Source
      .queue[(HttpRequest, Promise[HttpResponse])](512,
  ...
  /**
    ...
    * @return
    */
  def applyApiToken(request: HttpRequest,
                    appId: String,
                    appKey: String,
  ...
  /**
    ...
    * @return HttpResponse
    */
  def singleRequest(method: HttpMethod,
                    uri: Uri,
      ...
      entity = data match {
        ...
        case _ =>
          HttpEntity(ContentTypes.`application/json`,
                     Jackson.defaultObjectMapper.writeValueAsString(data))
      },
      protocol = protocol
    )
  ...
  /**
    ...
    */
  def hostRequest(request: HttpRequest)(
  ...
  def hostRequest(method: HttpMethod,
                  uri: Uri,
                  params: Seq[(String, String)] = Nil,
                  data: AnyRef = null,
                  headers: immutable.Seq[HttpHeader] = Nil)(
  ...
  def toStrictEntity(response: HttpResponse)(
      implicit mat: Materializer): HttpEntity.Strict =
    toStrictEntity(response.entity)

  def toByteString(response: HttpResponse)(
      implicit mat: ActorMaterializer): Future[ByteString] =
    Unmarshal(response.entity).to[ByteString]

  def toStrictEntity(responseEntity: ResponseEntity)(
      implicit mat: Materializer): HttpEntity.Strict = {
    import scala.concurrent.duration._
    val dr = 10.seconds
