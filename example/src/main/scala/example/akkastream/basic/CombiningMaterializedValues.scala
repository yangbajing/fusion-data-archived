...
object CombiningMaterializedValues extends App {
  ...
  private def preMaterialize() = {
    val matValuePoweredSource = Source.actorRef[String](10, OverflowStrategy.fail)
  ...
  private def basic() = {
    ...
    val rr3: RunnableGraph[Promise[Option[Int]]] = source.via(flow).toMat(sink)(Keep.left)
    val rrr3: RunnableGraph[(Promise[Option[Int]], Future[Int])] = source.via(flow).toMat(sink)(Keep.both)
