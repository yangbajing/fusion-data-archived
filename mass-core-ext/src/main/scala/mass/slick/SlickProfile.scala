...
trait SlickProfile
  ...
  override val jsonMethods: JsonMethods[DOCType] =
    org.json4s.jackson.JsonMethods.asInstanceOf[JsonMethods[DOCType]]
  ...
  object MyAPI
    ...
    implicit val durationColumnType: BaseColumnType[FiniteDuration] =
      MappedColumnType.base[FiniteDuration, Long](
        _.toMillis,
    ...
    implicit val json4sJsonArrayTypeMapper: AdvancedArrayJdbcType[JValue] =
      new AdvancedArrayJdbcType[JValue](
        ...
        s =>
          utils.SimpleArrayUtils
            ...
            .orNull,
        v =>
          utils.SimpleArrayUtils.mkString[JValue](j =>
            jsonMethods.compact(jsonMethods.render(j)))(v)
      )
    ...
    def dynamicFilter(item: Option[Rep[Boolean]],
  ...
  def createDatabase(configuration: Configuration): backend.DatabaseDef = {
    ...
    val registerMbeans =
      configuration.getOrElse[Boolean]("registerMbeans", false)
