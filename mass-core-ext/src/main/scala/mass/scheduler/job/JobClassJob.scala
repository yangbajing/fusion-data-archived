...
private[scheduler] class JobClassJob extends Job with StrictLogging {

  override def execute(context: JobExecutionContext): Unit = {
    val jobClass = context.getJobDetail.getJobDataMap.getString(SchedulerUtils.JOB_CLASS)
    require(StringUtils.isNoneBlank(jobClass), s"JOB_CLASS 不能为空。")

    Class.forName(jobClass).newInstance() match {
      case job: SchedulerJob =>
        val data = (context.getJobDetail.getJobDataMap.asScala.mapValues(_.toString) - SchedulerUtils.JOB_CLASS).toMap
      ...
      case unknown =>
        logger.error(s"未知的任务类型：${unknown.getClass.getName}，需要 ${classOf[SchedulerJob].getName} 的子类。")
    }
  }
